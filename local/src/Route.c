 ...
 */
struct Route {
	int dist;				//!< Distance reachable by this route.
	int depth;				//!< Depth (quality) of the route.
	Vector acceleration;	//!< Acceleration necessary to engage this route.
};

List Route_createFromCar(Car car, Map map, int depth)
{
	return Route_create(Car_getPosition(car), Car_getSpeed(car), Car_getBoosts(car), map, depth);
}

void Route_makeRoutes(Position position, Vector speed, int boosts, Map map, int depth, int limitDepth, List routes, Vector originAcceleration, int needBoosts)
{
	List positions;
	...
	positions = Map_getReachablePositions(map, position, speed, (needBoosts) ? boosts : 0);
	...
	List_foreach(positions, newPosition, i)
	{
		...
		if(!(Vector_squaredLength(newSpeed) == 0 && Vector_squaredLength(speed) == 0))
		{
			...
			route->dist = Map_getDistance(map, newPosition->x, newPosition->y);
			route->depth = depth;
			...
			Route_makeRoutes(newPosition, newSpeed, boosts - useBoost, map, depth + 1, limitDepth, routes, originAcceleration, needBoosts);
		}
...
/**
 * Determines whether using a boost is appropriate.
 * @param position Current position.
 * @param speed Current speed.
 * @param map Current map.
 * @param depth How many rounds must be predicted in advance.
 * @return 0 if a boost is not adviced, 1 otherwise.
 */
...
List Route_create(Position position, Vector speed, int boosts, Map map, int depth)
{
	List routes = List_new();
	int needBoosts = Route_needBoosts(position, speed, map, depth);

	Route_makeRoutes(position, speed, boosts, map, 1, depth, routes, NULL, needBoosts);
